...
- For bot trade-chain analysis, pin one cutoff and call raw `positions.episodes` `metrics` for `24h`, `7d`, `30d`, `90d`, `365d`, `all`, adaptive, and disjoint intervals. Start one all-history unselected `event_detail` read through `analysis.start`, then consume its complete immutable artifact and reconcile every decision-event, canonical-fill, raw-funding, and chunk count. Persisted matrix/index/detail projections and selected episode IDs are retired. Transport chunks are never permission to sample, rank-cap, censor, or silently truncate the authorized population.
