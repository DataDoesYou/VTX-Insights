...
- Choose one artifact mode. For targeted values, begin with `artifact.query path=[]`, recursively query each needed disclosed object path (following `next_structure_offset`), and query a disclosed list parent for `available_index_range` before batching exact children. Discovery is stateless; successful calls do not create index entries. Use `artifact.query_many` for several indexed descendants or typed filters over one exact indexed list/object. A large arbitrary predicate scan examines every member of that parent without materializing it or reading unrelated siblings, so submit the unchanged batch through `analysis.start` if it exceeds the response window. For complete retrieval, begin with `artifact.manifest`, read every advertised chunk, and acknowledge consecutive hashes through `artifact.resume`. Never mix query and complete modes on one handle.
