...
After updating, confirm the installed manifest reports only `2026.8.17` before
...
`claude plugin install vtx-insights@vtx-insights`. Confirm the installed manifest reports only `2026.8.17`, run `/reload-plugins`, and start a fresh session. Use `claude plugin disable`, `enable`, or `uninstall` with
